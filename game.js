...
const audio = {
    ...
    gunshot(){
        ...
        const len=(this.ctx.sampleRate*0.35)|0;
        const buf=this.ctx.createBuffer(1,len,this.ctx.sampleRate);
        const d=buf.getChannelData(0);
        for(let i=0;i<len;i++) d[i]=(Math.random()*2-1)*Math.exp(-i/(this.ctx.sampleRate*0.035));
        const src=this.ctx.createBufferSource(); src.buffer=buf;
        const lp=this.ctx.createBiquadFilter(); lp.type='lowpass';
        lp.frequency.setValueAtTime(5000,t); lp.frequency.exponentialRampToValueAtTime(80,t+0.35);
        const g=this.ctx.createGain(); g.gain.setValueAtTime(0.65,t); g.gain.exponentialRampToValueAtTime(0.001,t+0.35);
        src.connect(lp).connect(g).connect(this.ctx.destination); src.start(t);
        const osc=this.ctx.createOscillator(); osc.frequency.setValueAtTime(90,t); osc.frequency.exponentialRampToValueAtTime(18,t+0.25);
        const og=this.ctx.createGain(); og.gain.setValueAtTime(0.5,t); og.gain.exponentialRampToValueAtTime(0.001,t+0.25);
        osc.connect(og).connect(this.ctx.destination); osc.start(t); osc.stop(t+0.25);
        const del=this.ctx.createDelay(); del.delayTime.value=0.1;
        const eg=this.ctx.createGain(); eg.gain.value=0.12; g.connect(del).connect(eg).connect(this.ctx.destination);
    },
    bolt(){
        if(!this.ctx)return; const t=this.ctx.currentTime;
        const click=(delay,freq)=>{
            const n=(this.ctx.sampleRate*0.035)|0, b=this.ctx.createBuffer(1,n,this.ctx.sampleRate), d=b.getChannelData(0);
            for(let i=0;i<n;i++) d[i]=(Math.random()*2-1)*Math.exp(-i/(this.ctx.sampleRate*0.005));
            const s=this.ctx.createBufferSource(); s.buffer=b;
            const f=this.ctx.createBiquadFilter(); f.type='bandpass'; f.frequency.value=freq; f.Q.value=10;
            const g=this.ctx.createGain(); g.gain.value=0.22;
            s.connect(f).connect(g).connect(this.ctx.destination); s.start(t+delay);
        };
        click(0.30,3500); click(0.50,2600); click(0.95,2800); click(1.15,4000);
    },
    hitPing(){
        if(!this.ctx)return; const t=this.ctx.currentTime;
        const o1=this.ctx.createOscillator(); o1.frequency.setValueAtTime(1500,t); o1.frequency.setValueAtTime(1200,t+0.04);
        const g1=this.ctx.createGain(); g1.gain.setValueAtTime(0.22,t); g1.gain.exponentialRampToValueAtTime(0.001,t+0.12);
        o1.connect(g1).connect(this.ctx.destination); o1.start(t); o1.stop(t+0.12);
        const o2=this.ctx.createOscillator(); o2.frequency.value=850;
        const g2=this.ctx.createGain(); g2.gain.setValueAtTime(0.12,t+0.01); g2.gain.exponentialRampToValueAtTime(0.001,t+0.09);
        o2.connect(g2).connect(this.ctx.destination); o2.start(t+0.01); o2.stop(t+0.09);
    },
};
...
function makeMetalTex(){
    return canvasTex(128,128,(ctx,w,h)=>{
        ...
        for(let y=0;y<h;y++)for(let x=0;x<w;x++){
            const brushed=Math.sin(y*2.5+Math.random()*0.5)*5;
            const v=78+brushed+Math.random()*18;
            const i=(y*w+x)*4; d[i]=v; d[i+1]=v+1; d[i+2]=v+3; d[i+3]=255;
        }
        ctx.putImageData(img,0,0);
        for(let i=0;i<12;i++){
            const cx=Math.random()*w,cy=Math.random()*h,r=2+Math.random()*6;
            ctx.beginPath(); ctx.arc(cx,cy,r,0,Math.PI*2);
            ctx.fillStyle=`rgba(${140+Math.random()*40},${60+Math.random()*30},${20+Math.random()*20},0.25)`; ctx.fill();
        }
        for(let i=0;i<8;i++){
            ctx.strokeStyle='rgba(200,200,200,0.15)'; ctx.lineWidth=0.5;
            ctx.beginPath(); const sx=Math.random()*w;
            ctx.moveTo(sx,Math.random()*h); ctx.lineTo(sx+Math.random()*30-15,Math.random()*h); ctx.stroke();
        }
    });
}
function makeSandbagTex(){
    return canvasTex(128,128,(ctx,w,h)=>{
        const img=ctx.createImageData(w,h); const d=img.data;
        for(let y=0;y<h;y++)for(let x=0;x<w;x++){
            const weaveX=Math.sin(x*0.8)*8,weaveY=Math.sin(y*0.8)*8;
            const weave=(((x+weaveY|0)%6<3)!==((y+weaveX|0)%6<3))?12:0;
            const v=140+weave+Math.random()*20;
            const i=(y*w+x)*4; d[i]=v+5; d[i+1]=v-8; d[i+2]=v-28; d[i+3]=255;
        }
        ctx.putImageData(img,0,0);
        for(let i=0;i<4;i++){
            const sy=20+i*28;
            ctx.strokeStyle='rgba(90,70,45,0.3)'; ctx.lineWidth=1;
            ctx.beginPath(); ctx.moveTo(0,sy);
            for(let x=0;x<w;x+=4) ctx.lineTo(x,sy+(x%8<4?-1:1));
            ctx.stroke();
        }
    },2,2);
}
function makeRockTex(){
    return canvasTex(128,128,(ctx,w,h)=>{
        ...
        for(let i=0;i<w*h;i++){
            const x=i%w,y=(i/w)|0;
            const n=Math.sin(x*0.15+y*0.1)*12+Math.sin(x*0.3-y*0.2)*6;
            const v=95+n+Math.random()*25;
            d[i*4]=v; d[i*4+1]=v-2; d[i*4+2]=v-4; d[i*4+3]=255;
        }
        ctx.putImageData(img,0,0);
        for(let i=0;i<5;i++){
            ctx.strokeStyle='rgba(50,45,40,0.25)'; ctx.lineWidth=0.8;
            ctx.beginPath(); let cx=Math.random()*w,cy=Math.random()*h; ctx.moveTo(cx,cy);
            for(let j=0;j<6;j++){cx+=Math.random()*20-10;cy+=Math.random()*15;ctx.lineTo(cx,cy);}
            ctx.stroke();
        }
        for(let i=0;i<8;i++){
            ctx.fillStyle=`rgba(${80+Math.random()*30},${100+Math.random()*40},${60+Math.random()*20},0.15)`;
            ctx.beginPath(); ctx.arc(Math.random()*w,Math.random()*h,3+Math.random()*8,0,Math.PI*2); ctx.fill();
        }
    });
}
...
const _foldQuat=new THREE.Quaternion();
...
const mat=new THREE.Matrix4();
        mat.set(1,0,0,tx, 0,1,0,ty, 0,0,1,tz, 0,0,0,1);
        t.mesh.matrix.copy(mat);
...
const GRIP_TARGET       = new THREE.Vector3(0.16, -0.20, -0.38);
...
const ex = Math.sin(t)*r;
        const ey = 9+Math.sin(t*0.6)*2.5;
        const ez = Math.cos(t)*r*0.45-22;
        const E = new THREE.Vector3(ex, ey, ez);

const T = new THREE.Vector3(0, 2, -50);

const U = new THREE.Vector3(0, 1, 0);

const Z = new THREE.Vector3().subVectors(E, T).normalize();
